...
Redirect:
	...
	if ((host = gethostbyname(http_proxy_server ? http_proxy_server :
		http_server))) {
		memset((char *)&server, 0, sizeof(server));
		memmove((char *)&server.sin_addr, host->h_addr,
			host->h_length);
		server.sin_family = host->h_addrtype;
		server.sin_port = (unsigned short)htons(http_proxy_server ?
			http_proxy_port : po);
	...
	alarm(timeout);
	n = connect(s, (struct sockaddr *)&server, sizeof(server));
